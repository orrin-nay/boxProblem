...
for i in range(0,100):
    ...
    boxNumList.append(numberList[ranNumber])
    boxCheckedList.append(False)
    del numberList[ranNumber]


def runAGame(bNL,bCL):#boxNumList,boxCheckedList
...
def worker(newCMT, index):
    ...
    for run in range(0,int(tries/numOfthreds)):
        #print(run)
        holderCMT += runAGame(list(boxNumList), list(boxCheckedList))
